...
fig = go.Figure(data=[go.Bar(
    ...
    customdata = pd_consolidated['state'],
    hovertemplate = "City: %{text}<br>State: %{customdata}<br>Incident Description: %{hovertext}<br>Incident Date:%{x} <extra></extra>",
    showlegend = True,
)])
...
fig.update_layout(title_text = "2020 Police Brutality: {} Incidents Captured on Video<br>Data from {} to {}<br>Date Created: {} MST".format(len(pb_data_raw),first_date,last_date,dt_string),
                  #barmode='group',
                  xaxis_tickangle=0,
                  autosize=True,
...
fig.write_html(r"C:\Users\cdwhi\Documents\Python\My_Code\Police_Brutality_2020\PB_2020_Bar_Graph_Count_by_Date\index.html")
fig.show()
